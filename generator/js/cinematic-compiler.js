...
window.ArbelCinematicCompiler = (function () {
     ...
     *  User-supplied images/videos always arrive base64-encoded via
     *  FileReader.readAsDataURL, so we only need to allow that narrow shape.
     *  Inline-SVG data URLs (data:image/svg+xml;utf8,...) are rejected here
     *  because SVG can carry <foreignObject> / event handlers. Internal
     *  SVG data URIs generated by the compiler itself are embedded directly
     *  into HTML/CSS strings without going through escHref. */
    function escHref(url) {
        if (!url) return '';
        var s = url.replace(/[\x00-\x1f]+/g, '').trim();
        if (/^\s*(javascript|vbscript)\s*:/i.test(s)) return '';
        if (/^data:/i.test(s)) {
            // Allowlist: base64-encoded raster images or standard video containers only.
            if (!/^data:(image\/(jpeg|jpg|png|webp|gif)|video\/(mp4|webm|ogg));base64,[A-Za-z0-9+\/=]+$/i.test(s)) return '';
        }
        return esc(s);
    }
